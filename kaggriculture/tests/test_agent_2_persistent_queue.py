...
CANDIDATE = ROOT / "agents_2.0.x/round37_2b_persistent_queue"
...
def _ready_scheduler():
    ...
    scheduler = scheduler_module.PersistentQueueScheduler()
...
def test_unavailable_goal_invalidates_once_and_rebuilds_from_actual_choice():
    loaded, scheduler, graph = _ready_scheduler()
    candidates = (_candidate(1, 5, 2, (1, 0)),)
    assert scheduler.choose(0, candidates, (), 1) == 1
    scheduler.record_selector(0, 1, candidates, (), {})
    plan = scheduler.finish(graph, _policy(loaded).baseline.module)
    assert scheduler.rebuild_count == 1
    assert dict(scheduler.diagnostics.invalidations) == {"goal-unavailable": 1}
    assert plan.routes[0].head == graph.nodes[1].identifier
...
def test_duplicate_and_reset_keep_transaction_boundaries():
    ...
    candidate(copy.deepcopy(second))
    candidate(copy.deepcopy(first))
    assert policy.scheduler.rebuild_count == 1
...
def test_live_episode_uses_persistent_routes_across_walking_turns():
    ...
    assert scheduler.diagnostics.persisted_selections >= 100
    assert scheduler.rebuild_count < 720
