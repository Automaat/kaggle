...
def _pickup_op(pos, carried, shed, hungry, unplaced, n_units, board_size, needs_fertilizer, mine=None):
    ...
    # Every unit passing the shed used to take a share of the feed and the
    # fertilizer whether or not it had anywhere to carry it, so wheat rode
    # around in hand for eight thousand unit-turns to deliver two hundred
    # feeds. Load only what this unit's own work will consume.
...
def agent(obs):
    ...
    for idx, pos in enumerate(units):
        ...
        mine = None
        if PICKUP_ON_DEMAND:
            # What this unit's own strip still asks of it, in loads.
            mine = {"FEED": 0, "FERTILIZE": 0}
            for i, (prio, x, y, (task, _item)) in enumerate(tasks):
                if i in taken or (zones and zones.get((x, y)) not in (None, idx)):
                    continue
                if task in ("FEED", "FEED!"):
                    mine["FEED"] += 1
                elif task == "FERTILIZE":
        ...
        load = _pickup_op(
            ...
            pickup_fertilizer if PICKUP_BUDGET else fertilize_jobs, mine,
        )
        if load is not None:
