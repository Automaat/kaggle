...
class BaselinePolicy:
    def __init__(self, path=None):
        ...
        self._late_crop_cohort = None
        self._late_crop_started = set()
        self.reset()
    ...
    def reset(self) -> None:
        ...
        def plan(tiles, day, inventory, shops, board_size=10, budget=None, seeds=None):
            ...
            self._apply_crop_calendar(
                module, result, tiles, day, inventory, shops, board_size,
            )
    ...
    def _apply_crop_calendar(self, module, result, tiles, day, inventory, shops, board_size):
        if os.environ.get("AGENT2_CROP_CALENDAR", "0") != "1":
            return
        start = int(os.environ.get("AGENT2_CROP_CALENDAR_DAY", "18"))
        if day < start:
            return
        tile_by_position = {(x, y): tile for x, y, tile in tiles}
        if self._late_crop_cohort is None:
            limit = max(0, int(os.environ.get("AGENT2_CROP_CALENDAR_COHORT", "8")))
            sale_buffer = max(0, int(os.environ.get("AGENT2_CROP_CALENDAR_SALE_BUFFER", "1")))
            crops = [
                crop for crop in ("WHEAT", "CARROT")
                if day + module.LIFESPAN[crop] <= module.LAST_DAY - sale_buffer
            ]
            projected = {
                crop: module._projected_inventory(
                    inventory, shops, day, module.LIFESPAN[crop]
                ).get(crop, module.MARKET_I0)
                for crop in crops
            }
            for crop in result.values():
                if crop in projected:
            ...
            positions = [
                position for position, tile in tile_by_position.items()
                if tile is None and result.get(position) is None
            ]
            positions.sort(key=lambda position: (
                min(abs(position[0] - x) + abs(position[1] - y) for x, y in ports),
                position,
            ))
            minimum = float(os.environ.get("AGENT2_CROP_CALENDAR_MIN_VALUE", "0"))
            self._late_crop_cohort = {}
            for position in positions[:limit]:
                viable = [
                    crop for crop in crops
                    if module._crop_value(crop, projected, day) > minimum
                ]
                if not viable:
                    break
                crop = max(viable, key=lambda item: module._crop_value(item, projected, day))
                self._late_crop_cohort[position] = crop
                projected[crop] += module._effective_yield(crop)
        for position, crop in self._late_crop_cohort.items():
            tile = tile_by_position.get(position)
            if (isinstance(tile, dict) and tile.get("kind") == "PLANT"
                    and tile.get("crop") == crop):
                self._late_crop_started.add(position)
                continue
            if position in self._late_crop_started:
                result[position] = None
                continue
            latest = module.LAST_DAY - int(os.environ.get(
                "AGENT2_CROP_CALENDAR_SALE_BUFFER", "1"
            )) - module.LIFESPAN[crop]
            result[position] = crop if tile is None and day <= latest else None
