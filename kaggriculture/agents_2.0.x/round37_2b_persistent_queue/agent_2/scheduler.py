...
class PersistentQueueScheduler(RebuildQueueScheduler):
    def __init__(self):
        super().__init__()
    ...
    def capture(self, graph, protected, units, inventories) -> None:
        ...
        if plan.unit_count != len(units):
            self._require_rebuild("unit-count")
            return
        routes = []
        absent = 0
    ...
    def choose(
        ...
        if route is None:
            self._require_rebuild("missing-route")
            self._pending["fallback"] += 1
            return original_index
        goal = route.head
        ...
        if persisted is None or node.source_order in taken:
            self._require_rebuild("goal-unavailable")
            self._pending["fallback"] += 1
            return original_index
