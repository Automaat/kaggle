...
class StickyGoalScheduler(RebuildQueueScheduler):
    def __init__(self):
        super().__init__()
        self._committed_plan = None
    ...
    def choose(
        ...
        if route is None:
            ...
            self._pending["unrouted"] += 1
            return original_index
        node = self._graph.find(route.head)
        by_index = {candidate[0]: candidate for candidate in candidates}
        original = by_index[original_index]
        persisted = by_index.get(node.source_order) if node is not None else None
        if persisted is None or node.source_order in taken:
    ...
    def record_selector(
        ...
        node = next(
            (
                ...
                and graph_node.source_order == candidate[0]
            ),
            None,
        )
        if node is None:
    ...
    def finish(self, graph: TaskGraph, module) -> TurnPlan:
        if self._graph is None:
            self._graph = graph
        routes = {route.unit_index: route for route in self._mapped_routes}
        routes.update(self._enrollments)
        ordered = tuple(routes[index] for index in sorted(routes))
        assigned = {
            node.source_order
            for route in ordered
            for node in (self._graph.find(route.head),)
            if node is not None
        }
        unassigned = tuple(
            node.source_order
