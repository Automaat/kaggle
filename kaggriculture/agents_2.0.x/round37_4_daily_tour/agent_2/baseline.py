...
class BaselinePolicy:
    ...
    def reset(self) -> None:
        ...
        def capture(tasks, units, inventories):
            ...
            if self._scheduler is not None:
                self._scheduler.capture(
                    TaskGraph.from_legacy(self._current_day, self._captured_tasks),
                    result,
                    units,
                    inventories,
                )
        ...
        def select(player, step, candidates, tasks, taken, unit_index, targets):
            ...
            if self._scheduler is not None:
                self._scheduler.record_selector(unit_index)
            return chosen
    ...
    def decide(self, obs, scheduler=None) -> BaselineDecision:
        ...
        self._current_day = int(obs["day"])
        if scheduler is not None:
            scheduler.begin(obs)
        try:
            action = self.module.agent(obs)
            graph = TaskGraph.from_legacy(self._current_day, self._captured_tasks or ())
            if scheduler is not None:
                action = scheduler.rewrite(action, self.module)
