...
class BaselinePolicy:
    def __init__(self, path=None):
        ...
        self._feed_pickup_units = set()
        self._pickup_call_index = 0
        self.reset()
    ...
    def reset(self) -> None:
        ...
        original_pickup = module._pickup_op
        ...
        def select(player, step, candidates, tasks, taken, unit_index, targets):
            ...
            delivery = self._late_delivery_choice(candidates, unit_index)
        ...
        def pickup(*args, **kwargs):
            unit_index = self._pickup_call_index
            self._pickup_call_index += 1
            result = original_pickup(*args, **kwargs)
            if (os.environ.get("AGENT2_DISTRIBUTE_FEED", "0") != "1"
                    or not result or result[:2] != ["PICKUP", "WHEAT"]):
                return result
            if unit_index not in self._feed_pickup_units:
                return None
            configured_load = int(os.environ.get("AGENT2_FEED_LOAD", "0"))
            load = configured_load or max(1, math.ceil(
                self._feed_pickup_hungry / max(1, len(self._feed_pickup_units))
            ))
            return [result[0], result[1], min(result[2], load)]
    ...
    def decide(self, obs) -> BaselineDecision:
        ...
        try:
            ...
            self._prepare_feed_pickups(obs)
            self._pickup_call_index = 0
    ...
    def _prepare_feed_pickups(self, obs):
        if os.environ.get("AGENT2_DISTRIBUTE_FEED", "0") != "1":
            self._feed_pickup_units = set()
            return
        farm = obs["farms"][obs["player"]]
        ...
        self._feed_pickup_hungry = len(hungry)
        limit = int(os.environ.get("AGENT2_FEED_CARRIERS", "4"))
        board_size = len(farm["tiles"])
        slots = max(0, limit - len(carriers))
        ranked = sorted(
            (
                index for index in range(len(units))
                if index not in carriers
                and self.module._is_shed_adjacent(units[index], board_size)
            ),
            key=lambda index: (
                min(
                    (abs(units[index][0] - x) + abs(units[index][1] - y) for x, y in hungry),
                    default=0,
                ),
                index,
            ),
        )
        self._feed_pickup_units = carriers | set(ranked[:slots])

    def _late_delivery_choice(self, candidates, unit_index):
        if os.environ.get("AGENT2_LATE_DELIVERY", "0") != "1":
            return None
        hour = int(self._observation["hour"])
        start = int(os.environ.get("AGENT2_LATE_DELIVERY_HOUR", "18"))
        if hour < start or int(unit_index) not in self._wheat_carriers or not candidates:
            return None
        safe_class = min(candidate[-1] for candidate in candidates)
        safe = [candidate for candidate in candidates if candidate[-1] == safe_class]
        if any(candidate[4][0] == "WATER!" for candidate in safe):
            return None
        nearest = min(candidate[5] for candidate in safe)
        feed = [
            candidate for candidate in safe
            if candidate[5] == nearest and candidate[4][0] in {"FEED", "FEED!"}
        ]
        if not feed:
            return None
        return min(feed, key=lambda candidate: (candidate[5], candidate[1], candidate[0]))[0]
