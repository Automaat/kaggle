...
class BaselinePolicy:
    ...
    def reset(self) -> None:
        ...
        module.MAX_QUADRANTS = int(os.environ.get(
            "AGENT2_LAND_BUDGET_LAND", os.environ.get("AGENT2_LAND", "2")
        ))
        module.MAX_HANDS = int(os.environ.get(
            "AGENT2_LAND_BUDGET_MAX_HANDS", os.environ.get("AGENT2_MAX_HANDS", "12")
        ))
        ...
        original_land_orders = module._land_orders
        ...
        def land_orders(farm, day, empty_tiles):
            if (os.environ.get("AGENT2_SHORT_SEED_RESERVE", "0") != "1"
                    or module.DAIRY_LAND_COWS > 0):
                return original_land_orders(farm, day, empty_tiles)
            eligible = dict(farm)
            eligible["money"] = float("inf")
            if not original_land_orders(eligible, day, empty_tiles):
                return []
            bought = len(farm.get("unlocked_quadrants", ["NW"])) - 1
            quadrant_tiles = (len(farm["tiles"]) // 2) ** 2
            horizon = max(1, int(os.environ.get("AGENT2_SEED_RESERVE_HORIZON", "1")))
            hour = int(self._observation["hour"])
            turns = max(0, module.SEED_BUY_STOP_HOUR - hour)
            turns += (horizon - 1) * module.SEED_BUY_STOP_HOUR
            purchasable = turns * module.SEED_BUY_BATCH
            owned = sum(self._observation["private"].get("seeds", {}).values())
            reserve_tiles = max(0, min(empty_tiles + quadrant_tiles, purchasable) - owned)
            reserve = int(os.environ.get(
                "AGENT2_SEED_RESERVE_PER_SEED", str(module.SEED_RESERVE)
            ))
            needed = module.LAND_PRICES[bought] + reserve_tiles * reserve
        ...
        module._land_orders = land_orders
        self.module = module
        self._captured_tasks = None
        self._observation = None
        self._bundle_positions = {}
    ...
    def decide(self, obs) -> BaselineDecision:
        ...
        try:
            ...
            self._add_sale_funded_land(obs, action)
            self._add_sale_funded_feed(obs, action)
            self._set_post_land_hires(obs, action)
    ...
    def _add_sale_funded_land(self, obs, action):
        if os.environ.get("AGENT2_SALE_FUNDED_LAND", "0") != "1":
            return
        orders = list(action.get("market", []))
        ...
        proceeds = sum(
            self.module._sale_revenue(item, inventory.get(item, self.module.MARKET_I0), count)
            for _, item, count in sells
        )
        farm = obs["farms"][obs["player"]]
        funded = dict(farm)
        funded["money"] = farm["money"] + proceeds
        tiles = list(self.module._my_tiles(farm))
        empty_tiles = sum(tile is None for _, _, tile in tiles)
        if not self.module._land_orders(funded, obs["day"], empty_tiles):
            return
        others = [order for order in orders if not order or order[0] != "SELL"]
        action["market"] = sells + [["BUY_LAND"]] + others

    def _set_post_land_hires(self, obs, action):
        if os.environ.get("AGENT2_POST_LAND_HIRING", "0") != "1":
            return
        orders = list(action.get("market", []))
        if not any(order and order[0] == "BUY_LAND" for order in orders):
            return
        if obs["hour"] >= self.module.HIRE_HOURS:
            return
        farm = obs["farms"][obs["player"]]
        unlocked = sum(1 for _ in self.module._my_tiles(farm))
        quadrant_tiles = (len(farm["tiles"]) // 2) ** 2
        wanted = min(
            self.module.MAX_HANDS,
            max(4, round((unlocked + quadrant_tiles) * self.module.HANDS_PER_TILE)),
        )
        missing = max(0, wanted - farm.get("hires_today", 0))
        hires = min(missing, self.module.HIRES_PER_TURN)
        non_hires = [order for order in orders if not order or order[0] != "HIRE"]
        action["market"] = non_hires + [["HIRE"]] * hires
