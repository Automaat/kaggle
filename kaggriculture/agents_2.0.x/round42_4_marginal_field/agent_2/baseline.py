...
class BaselinePolicy:
    ...
    def reset(self) -> None:
        ...
        def plan(tiles, day, inventory, shops, board_size=10, budget=None, seeds=None):
            ...
            if os.environ.get("AGENT2_MARGINAL_FIELD", "0") == "1":
                plant_cap = self._marginal_plant_cap(
                    module, result, tiles, day, inventory, shops, board_size, plant_cap,
                )
            standing_plants = sum(
    ...
    @staticmethod
    def _marginal_plant_cap(module, plan, tiles, day, inventory, shops, board_size, base_cap):
        start_day = int(os.environ.get("AGENT2_MARGINAL_FIELD_DAY", "18"))
        maximum = int(os.environ.get("AGENT2_MARGINAL_FIELD_MAX", "63"))
        if day < start_day or base_cap <= 0 or maximum <= base_cap:
            return base_cap
        standing = sum(
            isinstance(tile, dict) and tile.get("kind") == "PLANT"
            for _x, _y, tile in tiles
        ...
        planned = [
            ...
            if crop in module.CROPS
        ]
        planned.sort(key=lambda item: (
            min(
                abs(item[0][0] - port[0]) + abs(item[0][1] - port[1])
                for port in ports
            ),
            item[0],
        ))
        base_new = max(0, base_cap - standing)
        projected = {
        ...
        for _position, crop in planned[:base_new]:
            projected[crop] += module._effective_yield(crop)
        accepted = 0
        work_price = float(os.environ.get("AGENT2_FIELD_WORK_PRICE", "40"))
        travel_price = float(os.environ.get("AGENT2_FIELD_TRAVEL_PRICE", "8"))
        for position, crop in planned[base_new:max(0, maximum - standing)]:
            units = module._effective_yield(crop)
            revenue = module._sale_revenue(crop, projected[crop], units)
            span = module.LIFESPAN[crop]
            harvests = len(module.PRODUCTION_AGES.get(crop, ())) or 1
            fertilize = (span + module.FERTILIZE_DAYS - 1) // module.FERTILIZE_DAYS
            operations = 1 + span + harvests
            if crop in module.PRODUCTION_AGES:
                operations += fertilize
            distance = min(
                abs(position[0] - port[0]) + abs(position[1] - port[1])
                for port in ports
            )
            margin = (
                revenue
                - module.CROPS[crop]["seed"]
                - operations * work_price
                - operations * distance * travel_price
            )
            if margin <= float(os.environ.get("AGENT2_FIELD_MARGIN", "0")):
                continue
            accepted += 1
            projected[crop] += units
        return min(maximum, base_cap + accepted)
